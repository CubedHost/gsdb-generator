...
class Generator {
  ...
  run(callback) {
    this.init((err) => {
      ...
      this.scrapeSources((err) => {
        if (err) return callback(err);

        this.prunePackages((err) => {
          if (err) return callback(err);
  ...
  loadSources(callback) {
    ...
    let requirePath = path.join(__dirname, '..', this.sourcesPath);

    dbSources.find().toArray(function(err, docs) {
      if (err) return callback(err);

      docs.forEach(function(sourceConfig) {
        let sourceFilePath = path.join(requirePath, sourceConfig._id);

        let { name, url } = sourceConfig;
        let source = new (require(sourceFilePath))(name, url, sourceConfig);
