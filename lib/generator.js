  ...
  async scrape(source) {
    return new Promise((resolve, reject) => {
      ...
      let onScrapeComplete = async (err, data) => {
        if (err) {
          log(err);
          // TODO: Shouldn't this reject with an error? Keeping old behavior
          //  of resolving with no data for now...
          return resolve();
        }
        ...
        try {
          ...
          await Promise.all(actions.map((action) => processAction(action)));

          log('Updating source meta');
          sourcesMeta.update({ _id: source.id }, data.meta, { upsert: true });
      ...
      const processAction = async (action) => {
        ...
        if (action.update) {
          return packages.update({ '_id': action.pack._id }, action.pack);
        } else {
          return packages.insert(action.pack);
        }
      };
