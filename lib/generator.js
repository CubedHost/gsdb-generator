...
class Generator {
  ...
  async loadSources() {
    ...
    for (const sourceConfig of docs) {
  ...
  async scrape(source) {
    ...
    try {
      ...
      for (const pkg of dataPackages) {
        ...
        const dbPkgCount = await packages.countDocuments({ _id: pkg._id });
        await processAction({
          pack: pkg,
          update: dbPkgCount > 0
        });
  ...
  async detectChanges(newPackage) {
    ...
    try {
      let [ oldPackage ] = await dbPackages.find({ _id }).toArray();
  ...
  async enqueueBuild(pkg, version) {
    ...
    if (AWS_SNS_TOPIC) {
      try {
        ...
        log(`Notified via SNS: ${message.id}-${version}`);
      } catch (err) {
        log(err);
      }
    }
