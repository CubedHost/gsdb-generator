...
class Generator {
  ...
  constructor({ aws, mongo, sourcesPath, oldPackageThreshold }) {
    ...
    // AWS configuration
    if (!aws.sqs) throw new Error('Missing AWS SQS configuration!');
    ...
    if (aws.sns) process.env.AWS_SNS_TOPIC = aws.sns.topic;
  }
  ...
  scrape(source, callback) {
    ...
    let onScrapeComplete = (err, data) => {
      ...
      async.eachSeries(data, ::this.enqueueChanges, () => {
  ...
  enqueueChanges(newPackage, callback) {
    let dbPackages = this.dbConnection.collection('packages');
    ...
    dbPackages
      ...
      .toArray((err, [ oldPackage ]) => {
        ...
        newVersions.forEach((version) => {
          ...
          if (typeof version !== 'undefined' && !isNew) return;
          ...
          let message = Object.assign({}, newPackage);
