...
const SLUG_ALT_REGEX = /projects\/([^/]+)\/files/;
...
class CurseSource extends Source {
    ...
    super.scrape((err, packages) => {
      ...
      series([
        ...
        packages = packages.filter(pkg => {
          const { curseSlug } = pkg.metadata;
          if (this.broken[curseSlug]) {
            this.log(`Skipping broken pack: ${curseSlug}`);
            return false;
          }
  ...
  scrapeVersionsForPackage(pkg, key, callback) {
    const { metadata } = pkg;
    const { curseSlug, curseId } = metadata;

    this.log(`Scraping versions for ${curseSlug}`);

    const fileListUrl  = `${CURSEFORGE_URL}/projects/${curseId}/files`;
    const options = {
      encoding: 'utf8',
      format: 'html'
    };
    ...
    this.request(fileListUrl, options, (err, $) => {
      if (err) {
        this.broken[curseSlug] = true;
        this.log(`Error while scraping ${fileListUrl}: ${err}`);
        return callback();
      }
      ...
      versionList.each((i, packVersion) => {
        // Skip versions without a server pack
        const hasServerPack = $(packVersion).find('.more-files-tag');
        if (!hasServerPack || !/Server Pack/.test($(hasServerPack).text())) {
          return;
        }
      ...
      (pkg, key, next) => {
        ...
        eachOfLimit(
          ...
          (version, key, nextVersion) =>
            ...
            this.scrapeOrigin(pkg, version, key, nextVersion),
          next
        )
      },
  ...
  scrapeOrigin(pkg, version, key, callback) {
    const { curseSlug, curseId } = pkg.metadata;
    this.log(`Scraping origin URL for ${curseSlug} ${version.version}`);

    let fileUrl = `${CURSEFORGE_URL}/projects/${curseId}/files/${key}`;
    let options = {
      encoding: 'utf8',
      format: 'html'
    };
