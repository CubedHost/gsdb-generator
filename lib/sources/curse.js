...
import url from 'url';
import { series, mapSeries, mapValuesLimit, mapLimit } from 'async';
...
class CurseSource extends Source {
  ...
  static buildVersion(file) {
    return {
      id: file.id,
      version: CurseSource.formatVersion(file),
      minecraftVersion: file.gameVersion[0]
    };
  }
  ...
  process(addons) {
    ...
    addons = addons.map(originalPack => {
      ...
      files.forEach(file => {
        ...
        const newerFile = latestVersion < file.id && file.releaseType === 1;
        if (!latestVersion || newerFile) {
      ...
      mapSeries(files, (addFile, next) => {
        ...
        this.request(addFileUrl, options, (err, $) => {
          ...
          if (filename.match(/.*[sS]erver.*\.zip/i)) {
            return next(null, `${CURSEFORGE_URL}${href}`);
          }
        ...
        const origin = origins.filter(_ => _).shift();
