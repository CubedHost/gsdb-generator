...
export default class PocketMineSource extends GitHubSource {
  ...
  async process(result) {
    ...
    for (const release of github) {
      ...
      if (poggit[pmApiVersion]) {
        ...
        phpVersion = await this.getFallbackPhpVersion(pmVersion);
      }
      ...
      for (const version of gsdbRelease.minecraftVersions) {
        if (!data[version]) {
          data[version] = {
  ...
  async fetch() {
    return {
      github: ::this.fetchRemote(),
      //poggit: ::this.fetchPoggit()
    };
  }

  async fetchPoggit() {
    return await this.request('pmapis.json', {
      format: 'json',
      timeout: 10000
    });
  }
