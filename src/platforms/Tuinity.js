...
export default class TuinityPlatform extends GitHubPlatform {
  jobs = [];
  tree = 'jobs[name,url,builds[actions[moduleRecords[mainArtifact[artifactId,canonicalName,fileName,version]]],number,result,url,artifacts[fileName,displayPath,relativePath],mavenArtifacts[moduleRecords[mainArtifact[fileName,version]]]],firstBuild]';
    ...
    for (const job of jobs) {
      for (const build of job.builds) {
        ...
        res.push({
          ...
          ...build,
          job,
          origin: `${build.url}/artifact/${paperArtifact[0].fileName}`
        });
      }
    }
